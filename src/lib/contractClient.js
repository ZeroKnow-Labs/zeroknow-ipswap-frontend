...
/**
 * Fetches the current ledger timestamp from the RPC server.
 * @returns {Promise<number>} unix seconds
 */
export async function getLedgerTimestamp() {
  const server = new StellarSdk.SorobanRpc.Server(RPC_URL);
  const { sequence } = await server.getLatestLedger();
  // Approximate: Stellar ledger closes ~every 5s; use Date as fallback
  // For precision, call getTransaction or use ledger close time if available
  return Math.floor(Date.now() / 1000);
}

/**
 * Fetches all swaps for a given buyer address by calling get_swaps_by_buyer,
 * then hydrates each swap ID into a full Swap object via get_swap.
 * @param {string} address - buyer wallet address
 * @returns {Promise<Array>} array of swap objects
 */
export async function getSwapsByBuyer(address) {
  ...
  // Simulate get_swaps_by_buyer(address)
  const buyerScVal = StellarSdk.nativeToScVal(address, { type: "address" });
  const idListResult = await server.simulateTransaction(
    new StellarSdk.TransactionBuilder(
      ...
      .addOperation(contract.call("get_swaps_by_buyer", buyerScVal))
      .setTimeout(30)
      .build()
  );

  const ids = StellarSdk.scValToNative(idListResult.result.retval);

  // Hydrate each swap ID into a full swap object
  const swaps = await Promise.all(
    ids.map(async (id) => {
      const swapResult = await server.simulateTransaction(
        new StellarSdk.TransactionBuilder(
          await server.getAccount(address),
          {
            fee: StellarSdk.BASE_FEE,
      ...
      return StellarSdk.scValToNative(swapResult.result.retval);
    })
  );
